...
<body>
	...
	if (session.getAttribute("csvo") != null) {
		csvo = (CustomerCenterVO) session.getAttribute("csvo");
	}
	DAO dao = new DAO();
	ArrayList<CustomerCenterVO> csarr = dao.CsCenterSel();
	%>
...
%>
	...
	int total_record = csarr.size();
	System.out.print(csarr.size());
...
<main class="cd-main-content">
 ...
 %>
		...
		<%
		} else if (csarr.size() < (pageno) * 5) {
		%>
		<div class="bo_reslist">
			<li>
			
				<%
				for (int i = 0; i < 5 - ((pageno) * 5 - csarr.size()); i++) {
				%> <%
 String result = csarr.get(i + (pageno - 1) * 5).getCs_art_reg_date().substring(5, 11);
 %>
				<div class="s-number">
 ...
 %>		</div>
        ...
        <div class="bo_fx">
            ...
            <li><a href="oneboard.jsp.jsp" class="btn_b03 btn"><i class="fa fa-pencil" aria-hidden="true"></i> 글쓰기</a></li>        </ul>	
            </div>
            

 <div class="paginate" style="text-align:center; letter-spacing:10px;">
 <a href="oneboard.jsp.jsp?pageno=<%=prev_pageno%>"> < </a>
	<%
	for (int i = page_sno; i <= page_eno; i++) {
	%>
	<a href="oneboard.jsp?pageno=<%=i%>"> <%
 if (pageno == i) {
 ...
 %>
	...
	<a href="oneboard.jsp?pageno=<%=next_pageno%>">></a>
</div>
